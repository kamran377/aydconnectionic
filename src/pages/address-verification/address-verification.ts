 ...
 */
@Component({
  selector: 'page-address-verification',
  templateUrl: 'address-verification.html',
})
...
export class AddressVerificationPage {
	...
	submitInfo() {
		...
		if(this.imageStr) {
			...
			loading.present();
			me.apiService.saveAddressVerificationRequest(me.imageStr, me.token)
				.then(function(data){
					loading.dismiss();
			...
			let toast = toastCtrl.create({
				message: "Select utility bill image to continue",
				duration: 3000,
				cssClass: 'toast-error',
				position:'top',
			});
